...
public class AISuggestionsController : ControllerBase
{
    ...
    [HttpPost("callback")]
    public async Task<ActionResult<AISuggestionResponseDto>> CreateAISuggestion(
        [FromBody] AISuggestionCreateDto dto)
    {
        try
        {
    ...
    public async Task<ActionResult<AISuggestionResponseDto>> GetAISuggestionByComplaintId(int complaintId)
    {
        try
        {
            ...
            if (aiSuggestion == null) return NotFound(new { message = "ไม่พบคำแนะนำ AI สำหรับเรื่องร้องเรียนนี้" });
