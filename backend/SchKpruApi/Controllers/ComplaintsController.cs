...
namespace SchKpruApi.Controllers
{
    ...
    public class ComplaintsController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult<ComplaintResponseDto>> CreateComplaint([FromBody] ComplaintCreateDto complaintCreateDto)
        {
            try
            {
        ...
        [HttpPut("{id}")]
        [Authorize]
        public async Task<ActionResult<ComplaintResponseDto>> UpdateComplaint(int id, [FromBody] ComplaintUpdateDto complaintUpdateDto)
        {
            try
            {
                var currentUserIdClaim = User.FindFirst("UserId")?.Value;
        ...
        [HttpGet("recent")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<ComplaintResponseDto>>> GetRecentComplaints([FromQuery] int count = 10)
        {
            try
            {
        ...
        [HttpPost("{id}/assign")]
        [Authorize]
        public async Task<ActionResult<ComplaintAssignmentResponseDto>> AssignComplaint(int id, [FromBody] ComplaintAssignmentCreateDto assignmentDto)
        {
            try
            {
                var currentUserIdClaim = User.FindFirst("UserId")?.Value;
                ...
                {
                    if (assignmentDto.AssignedToDeptId.HasValue && assignmentDto.AssignedToDeptId.Value != user.DepartmentId)
        ...
        [HttpPut("{id}/status")]
        [Authorize]
        public async Task<ActionResult<ComplaintResponseDto>> UpdateComplaintStatus(int id, [FromBody] ComplaintStatusUpdateDto statusUpdateDto)
        {
            try
            {
                var currentUserIdClaim = User.FindFirst("UserId")?.Value;
        ...
        [Authorize]
        public async Task<ActionResult<IEnumerable<RecentComplaintDto>>> GetRecentComplaintsForDashboard([FromQuery] int count = 5)
        {
            try
            {
                // Get current user info
