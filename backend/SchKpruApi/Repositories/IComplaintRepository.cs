...
namespace SchKpruApi.Repositories
{
    public interface IComplaintRepository : IGenericRepository<Complaint>
    {
        ...
        Task<(IEnumerable<Complaint>, int)> GetFilteredByUserRoleAsync(string? searchTerm, string? status, int page, int pageSize, int userId, string roleName, int? departmentId, int? groupId);
