    ...
    public class WebhookService : IWebhookService
    {
        ...
        public async Task SendComplaintCreatedWebhookAsync(ComplaintResponseDto complaint)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Queued complaint created webhook for ComplaintId: {ComplaintId}", complaint.ComplaintId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to queue complaint created webhook for ComplaintId: {ComplaintId}", complaint.ComplaintId);
            }
        }
    }
}