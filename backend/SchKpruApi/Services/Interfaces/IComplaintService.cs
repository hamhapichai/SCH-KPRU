...
public interface IComplaintService
{
    ...
    Task<ComplaintResponseDto?>
        UpdateComplaintAsync(int id, ComplaintUpdateDto complaintUpdateDto, int updatedByUserId);
    ...
    Task<(IEnumerable<ComplaintResponseDto>, int)> GetFilteredComplaintsAsync(string? searchTerm, string? status,
        int page, int pageSize);

    Task<IEnumerable<ComplaintResponseDto>> GetComplaintsByUserRoleAsync(int userId, string roleName, int? departmentId,
        int? groupId);

    Task<(IEnumerable<ComplaintResponseDto>, int)> GetFilteredComplaintsByUserRoleAsync(string? searchTerm,
        string? status, int page, int pageSize, int userId, string roleName, int? departmentId, int? groupId);

    // Assignment methods
    Task<ComplaintAssignmentResponseDto?> AssignComplaintAsync(int complaintId,
    ...
    Task<ComplaintResponseDto?> UpdateComplaintStatusAsync(int id, ComplaintStatusUpdateDto statusUpdateDto,
        int updatedByUserId);
    ...
    Task<IEnumerable<RecentComplaintDto>> GetRecentComplaintsForDashboardAsync(int count, int userId, string roleName,
